...
const DashboardPage = () => {
  ...
  // Fetch análisis y la última fecha de envío
  const fetchData = async () => {
    try {
      ...
      // Obtener última fecha de envío de backend
  ...
  const handleEnviarCorreo = async () => {
    ...
    try {
      ...
      await Promise.all(
        medicosEmails.map((email) =>
          emailjs.send(
            'service_60i6uo8',
            'template_q39v373',
            {
              alertas: contenidoCorreo,
              to_email: email,
            },
            'kJOuxWlwaHHnVYWmL'
          )
      ...
      // Actualizar en backend el último envío
